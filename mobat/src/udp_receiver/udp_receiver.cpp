...
void UdpReceiver::draw(sf::RenderWindow& canvas){
	for(auto & current_position : positions_){
		current_position.second.recalculateGeometry();
		current_position.second.draw(canvas);
	}
}

glm::vec2 UdpReceiver::directionFromTo(unsigned ID1, unsigned ID2) const{
	return glm::normalize(glm::vec2{positions_.at(ID2).physical_position_.x 
                                - positions_.at(ID1).physical_position_.x
							         , positions_.at(ID2).physical_position_.y 
                       - positions_.at(ID1).physical_position_.y});
}
