...
Game(std::vector<glm::vec2> const& microphone_positions,
  ...
  // parameters for keyboard controlled game
      ...
      for(auto const& i : recognized_tokens_){
        ...
        if(move_ball_ || ball_reset_) {
          if(ball_reset_) {
            ...
            if(!already_moved_out) {
            // only recalculate ball speed while ball is slowing down
...
void Game::moveBallOutOfToken(Token const& paddle, glm::vec2 const& direction){    
  glm::vec2 mid_paddle{paddle.getPhysicalPosition()};
...
std::pair<bool, std::string> Game::gameOver(){
  int goal_difference = left_goals_ - right_goals_;
  if(abs(goal_difference) < points_.getMaxPoints() / 2){
    return {false, "-"};
  }else{
...
void Game::handleKeyboardInput() {
  glm::vec2 field_max{ DrawableObject::physical_projection_offset_ + 
    DrawableObject::physical_projection_size_ - glm::vec2{0.02f, 0.02f}};
  glm::vec2 field_min{DrawableObject::physical_projection_offset_ 
