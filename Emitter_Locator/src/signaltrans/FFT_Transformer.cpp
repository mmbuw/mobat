...
void FFT_Transformer::perform_FFT_on_channels(buffer_collection const& signal_buffers, unsigned window_size, unsigned signal_half_chunk) {
  ...
  for (unsigned int channel_iterator = 0; channel_iterator < 4; ++channel_iterator) {
    set_FFT_buffers(4, signal_buffers.length, signal_buffers[channel_iterator]);
    ...
    for(unsigned int i = signal_chunk * (signal_buffers.length/num_chunks) ; i < (signal_chunk+1)*(signal_buffers.length/num_chunks) - 50; ++i) {
      unsigned offset = 1 * i;
      if(offset > (signal_chunk+1)*(signal_buffers.length/num_chunks) - 50 )
        break;
