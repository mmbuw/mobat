...
void FFT_Transformer::perform_FFT_on_channels(buffer_collection const& signal_buffers, unsigned ints_per_channel, unsigned window_size, unsigned signal_half_chunk) {
	...
	ints_per_channel_ = ints_per_channel;
    ...
    for (unsigned int channel_iterator = 0; channel_iterator < 4; ++channel_iterator) {
        ...
        std::thread t1( [&] { this->perform_FFT_on_certain_channel(channel_iterator); } );
		//perform_FFT_on_certain_channel(signal_buffers,ints_per_channel,window_size,signal_half_chunk, channel_iterator, num_chunks);
    		t1.join();
    }
*/
    ...
    {
    	//std::cout << "Still waiting for threads \n";
    }
